...
export default async function Project({ params }) {
  ...
  const linkedPosts =
    projectSettings.linkedPosts?.length > 0
      ? await Promise.all(
          projectSettings.linkedPosts.map(async (postId) => {
    ...
    <div id="ProjectPage">
      <div className="box py-12">
        <div className="flex flex-col">
          <BlurredSidesImg
            className="mb-8"
            src={projectSettings.cover?.png || projectSettings.cover}
            webp={projectSettings.cover?.webp}
            alt={`Cover image for project "${projectSettings.title}"`}
            imgClassName="max-h-[200px]"
          />
          <h1 className="text-3xl font-accent mb-6">{projectSettings.title}</h1>
          ...
          {linkedPosts.length > 0 && (
            <>
              <h2 className="text-2xl font-accent mb-6">Blog posts</h2>
              <BlogPostsList
                posts={linkedPosts}
                addSeeMore={false}
                className="mb-6"
          ...
          {projectSettings.otherImages?.length > 0 && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {projectSettings.otherImages?.map((imageData, index) => {
                  return (
                    <ClickableImage
