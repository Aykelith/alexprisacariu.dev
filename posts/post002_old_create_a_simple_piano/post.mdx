...
First, we create a loading function which will load the sounds from the folder “sounds” into an array of sf::SoundBuffer/s:
...
Next we need to setup the piano. Because the piano keys are simple rectangles, we will use a vector of sf::RectangleShape/s for keys: the first 7 rectangles are the white keys and the rest of 5 rectangles are the black keys.
