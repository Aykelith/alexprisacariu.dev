...
seoTitle: "Implementing Jumping Mechanics in 2D Platformers with SFML and C++"
seoDescription: "Learn how to implement smooth jumping mechanics in a 2D platform game using C++ and SFML. Covers gravity, ground detection, and jump physics."
seoKeywords:
   - SFML
   - C++
   - jumping mechanics
...
We set the variable `onGround` initially to `false` because we set the player in the air.

We set the variable to false every frame because we don’t know if the player is still colliding with a block of grass. If the player collides with a block of grass we set the value to `true`.
...
Otherwise we set the y value of velocity to gravity, so the player will fall down until they hit a block of grass, then we set `inAir` to `maxInAir` so the player can’t jump from air.
