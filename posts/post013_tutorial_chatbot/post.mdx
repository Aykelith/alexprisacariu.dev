...
we will use ChatGPT, for the STT (speech-to-text) [[Willow]] and for TTS 
(text-to-speech) we will use [[X]].
...
The application will be implemented in JavaScript (ECMAScript), but I will also
show how the implementation would look like in Go.
...
implementation do `npm run start-v2`.
...
[[naive_version_activity_diagram.drawio.svg]]
...
app.post('/api/message', async (req, res) => {
    ...
    try {
        ...
        let audio;
        if (Settings.CreateAudioOfAnswer) audio = await tts(openai, answer);
